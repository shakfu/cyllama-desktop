...
Looks up the `## [<version>]` heading in CHANGELOG.md, with or without a
trailing date (`## [0.1.0] - 2026-04-25`), captures the block up to the
next `## [` heading, strips leading/trailing blank lines, prepends a
`## Changes since the last Release` header (configurable), and writes
...
Used by .github/workflows/build.yml to populate the GitHub release body.
Adapted from chimera's scripts/release_notes.py, which matches bare
headings only.
"""
...
def extract_section(changelog_text: str, version: str) -> Optional[str]:
    ...
    for line in changelog_text.splitlines():
        if not in_section and (line == header or line.startswith(header + " ")):
            in_section = True
            continue
